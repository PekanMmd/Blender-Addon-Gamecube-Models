...
class DATParser(BinaryReader):
    ...
    def read(self, field_type, address, offset=0, relative_to_header=True, whence='start'):
        if self.options.get("verbose"):
            print("reading field type:", field_type, " at:",
                  hex(address + offset + self._startOffset(relative_to_header)))
        ...
        if isBracketedType(field_type):
            ...
            # TODO: figure out what implications this has for the builder
            if pointer == 0:
            ...
            while True:
                ...
                raw_bytes = self.read_chunk(sub_type_length, address,
                                            current_offset + self._startOffset(relative_to_header))
...
class DATBuilder(BinaryWriter):
    ...
    def writeNode(self, node, relative_to_header, fields=None):
        ...
        for field in fields:
            ...
            if isBracketedType(field_type):
                field_type = getBracketedSubType(field_type)
